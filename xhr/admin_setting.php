...
if ($f == 'admin_setting' AND (Wo_IsAdmin() || Wo_IsModerator())) {
    ...
    if ($s == 'verfiy_apps') {
        ...
        if (!empty($_POST['android_purchase_code'])) {
            ...
            $file         = file_get_contents("http://www.socialfi.com/access_token.php?code={$android_code}&type=android", false, stream_context_create($arrContextOptions));
        ...
        if (!empty($_POST['windows_purchase_code'])) {
            ...
            $file         = file_get_contents("http://www.socialfi.com/access_token.php?code={$windows_code}&type=windows_desktop", false, stream_context_create($arrContextOptions));
            $check        = json_decode($file, true);
            if (!empty($check['status'])) {
                if ($check['status'] == 'SUCCESS') {
        ...
        if (!empty($_POST['ios_purchase_code'])) {
            ...
            $file         = file_get_contents("http://www.socialfi.com/access_token.php?code={$windows_code}&type=ios", false, stream_context_create($arrContextOptions));
            $check        = json_decode($file, true);
            if (!empty($check['status'])) {
                if ($check['status'] == 'SUCCESS') {
    ...
    if ($s == 'add_reaction') {
        ...
        if (!empty($_FILES['socialfi'])) {
            ...
            if ($add == true) {
                if (!empty($_FILES['socialfi'])) {
                    $fileInfo = array(
                        'file' => $_FILES["socialfi"]["tmp_name"],
                        'name' => $_FILES['socialfi']['name'],
                        'size' => $_FILES["socialfi"]["size"],
                        'type' => $_FILES["socialfi"]["type"],
                        'types' => 'png'
                    );
                    $media    = Wo_ShareFile($fileInfo, true);
                    if (!empty($media) && !empty($media['filename'])) {
                        $socialfi_image = $media['filename'];
                    } else {
                        $data['status']  = 400;
                        $data['message'] = $error_icon . " socialfi image type must be png ";
                        header("Content-type: application/json");
                        echo json_encode($data);
                        exit();
                    }
                }
                if (!empty($socialfi_image)) {
                    $db->insert(T_REACTIONS_TYPES, array(
                        'name' => $id,
                        'socialfi_icon' => $socialfi_image
                    ));
                    $data = array(
                        'status' => 200
                    );
    ...
    if ($s == 'delete_reaction') {
        ...
        if (!empty($_POST['id']) && is_numeric($_POST['id']) && $_POST['id'] > 0) {
            ...
            if ($id > 6 && !empty($reaction)) {
                $explode2       = @end(explode('.', $reaction->socialfi_icon));
                $explode3       = @explode('.', $reaction->socialfi_icon);
                $socialfi_small = $explode3[0] . '_small.' . $explode2;
                if (file_exists($socialfi_small)) {
                    @unlink(trim($socialfi_small));
                } else if ($wo['config']['amazone_s3'] == 1 || $wo['config']['wasabi_storage'] == 1 || $wo['config']['ftp_upload'] == 1 || $wo['config']['backblaze_storage'] == 1) {
                    @Wo_DeleteFromToS3($socialfi_small);
                }
                if (file_exists($reaction->socialfi_icon)) {
                    @unlink(trim($reaction->socialfi_icon));
                } else if ($wo['config']['amazone_s3'] == 1 || $wo['config']['wasabi_storage'] == 1 || $wo['config']['ftp_upload'] == 1 || $wo['config']['backblaze_storage'] == 1) {
                    @Wo_DeleteFromToS3($reaction->socialfi_icon);
                }
    ...
    if ($s == 'get_reaction_form') {
        ...
        if (!empty($_POST['id']) && is_numeric($_POST['id']) && $_POST['id'] > 0) {
            ...
            if (!empty($reaction)) {
                ...
                $wo['socialfi_icon'] = $reaction->socialfi_icon;
    ...
    if ($s == 'edit_reaction') {
        ...
        if (!empty($_POST['id']) && is_numeric($_POST['id']) && $_POST['id'] > 0) {
            ...
            if (!empty($reaction)) {
                ...
                $update_data = array();
                if (!empty($_FILES['socialfi'])) {
                    // $cover = getimagesize($_FILES["socialfi"]["tmp_name"]);
                    // if (!empty($cover) && ($cover[0] > 48 || $cover[1] > 48)) {
                    //     $data['status']  = 400;
                    //     $data['message'] = $error_icon . " socialfi image size should not be more than 48x48 ";
                    //     header("Content-type: application/json");
                    //     echo json_encode($data);
                    //     exit();
                    // }
                    ...
                    $media    = Wo_ShareFile($fileInfo, true);
                    if (!empty($media) && !empty($media['filename'])) {
                        $update_data['socialfi_icon'] = $media['filename'];
                    } else {
                        $data['status']  = 400;
                        $data['message'] = $error_icon . " socialfi image type must be png ";
                        header("Content-type: application/json");
                ...
                if (!empty($_POST['socialfi_to_use']) && $_POST['socialfi_to_use'] == 1) {
                    $explode2       = @end(explode('.', $reaction->socialfi_icon));
                    $explode3       = @explode('.', $reaction->socialfi_icon);
                    $socialfi_small = $explode3[0] . '_small.' . $explode2;
                    if (file_exists($reaction->socialfi_icon)) {
