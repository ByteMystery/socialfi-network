        ...
        if (empty($user_login_data)) {
            ...
            if (empty($event_data)) {
                $json_error_data = array(
                    ...
                    'errors' => array(
                        'error_id' => '6',
                        'error_text' => 'Event is not exists.'
                    )
                );
                header("Content-type: application/json");
                echo json_encode($json_error_data, JSON_PRETTY_PRINT);
            ...
            if (Wo_EventInterestedExists($event_id) === true) {
                if (Wo_UnsetEventInterestedUsers($event_id)) {
                    $follow_data = array('interested' => false);
                }
            } else {
                if (Wo_AddEventInterestedUsers($event_id)) {
                    $follow_data = array('interested' => true);
                }
            }
        }
    } else {
