...
if ($type == 'update_page' || $type == 'u_page') {
                ...
                if (is_array($json_decode)) {
                    if (!empty($_POST['type'])) {
                        if ($_POST['type'] == 'general_settings') {
                            ...
                            if ($user_data['page_name'] != $pagedata['page_name']) {
                                $is_exist = Wo_IsNameExist($user_data['page_name'], 0);
                                if (in_array(true, $is_exist)) {
                                    $errors[] = $wo['lang']['page_name_exists'];
                                }
                                if (in_array($user_data['page_name'], $wo['site_pages'])) {
                            ...
                            if (empty($errors)) {
                                $u_user_data = array(
                                    ...
                                    'page_category' => $user_data['page_category']
                                );
                                $update_user_data = Wo_UpdatePageData($page_id, $u_user_data);
                                if ($update_user_data) {
                                    $updated = true;
                                }
                            }
				            ...
				            $upload_image = Wo_UploadImage($_FILES["image"]["tmp_name"], $_FILES['image']['name'], $image_type, $_FILES['image']['type'], $page_id, 'page');
				            if ($upload_image === true) {
				                $pageData = Wo_PageData($page_id);
                            ...
                            $user_data = $json_decode;
                            if (empty($errors)) {
                                $update_page_data = Wo_UpdatePageData($page_id, $user_data);
                                if ($update_page_data) {
                                    $updated = true;
                                }
                            }
