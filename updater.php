...
if ($f == 'run_updater') {
    ...
    if (!empty($_GET['purchase_code'])) {
        ...
        $file = file_get_contents("http://www.socialfi.com/check_for_updates.php?code={$purchase_code}&version=$version&url=$siteurl&check_for=true", false, stream_context_create($arrContextOptions));
        $check = json_decode($file, true);
        if (!empty($check['status'])) {
            if ($check['status'] == 'SUCCESS') {
                if (!empty($check['versions'])) {
                    foreach ($check['versions'] as $key => $version) {
                        $updater = file_put_contents('script.zip', file_get_contents("https://www.socialfi.com/get_update.php?code={$purchase_code}&version=$version&url=$siteurl", false, stream_context_create($arrContextOptions)));
                        if ($updater && file_exists('script.zip')) {
                            $updater = file_put_contents('theme.zip', file_get_contents("https://www.socialfi.com/get_update.php?code={$purchase_code}&version=$version&theme=true&url=$siteurl", false, stream_context_create($arrContextOptions)));
                            $updater = file_put_contents('wonderful.zip', file_get_contents("https://www.socialfi.com/get_update.php?code={$purchase_code}&version=$version&wonderful=true&url=$siteurl", false, stream_context_create($arrContextOptions)));
                            if ($updater) {
                                $unzip_file = unzip_file('script.zip', './');
                                if ($unzip_file) {
                                    if (file_exists('theme.zip') && file_exists('./themes/socialfi')) {
                                        $unzip_file = unzip_file('theme.zip', './themes/socialfi/');
                                    }
